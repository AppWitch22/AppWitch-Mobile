...
// NOTA Step B5: DB e PRESETS restano globals (non migrati a store).
// Sono "database in memoria" riempiti una volta da initDB/loadPresets e letti
// da 200+ siti via indicizzazione diretta (DB[cod], PRESETS[…]). Non hanno
// subscriber, non cambiano tra schermate: migrarli a store.* aumenterebbe la
// cerimonia (store.get ovunque) senza eliminare debito reale.
let DB={};
let PRESETS={};

const saved=store.get('session.saved');
...
// currentUser: proxy su store.user.current (Step B5).
// Source of truth unica; i ~57 callsite continuano a funzionare perché
// window.currentUser è risolto via scope globale e va al getter.
Object.defineProperty(window, 'currentUser', {
  get() { return store.get('user.current'); },
  set(v) { store.set('user.current', v); },
  configurable: true,
  enumerable: true,
});
...
// NOTA: syncScheda / syncSessionNow spostate in js/sync.js (Step B3).
...
// NOTA: collectVSEFromRec/MPFromRec/VSPFromRec/CQFromRec spostati in js/sync.js (Step B3).

// NOTA: updateSyncBar / scheduleSync spostate in js/sync.js (Step B3).
