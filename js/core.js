...
let DB={};let cur=null;let dirty=false;const saved={};
// curVerif vive in store.session.curVerif
...
function sel(cod){
  ...
  if(_cv&&_cv.vsp){buildVSPPoints(_cv.vsp);fillVSPHeader(cur);}
  if(_cv&&_cv.cq){buildCQPoints(_cv.cq);fillCQHeader(cur);}
  updateTabs();
  switchTab('vse');
  ...
  if(hasSaved){
    ...
    if(_cv2&&_cv2.vsp) setTimeout(()=>fillVSPPreset(cur.c,_cv2.vsp),50);
    if(_cv2&&_cv2.cq)  setTimeout(()=>fillCQPreset(cur.c,_cv2.cq),50);
  }
...
async function removeFromSession(cod) {
  ...
  delete saved[cod];
  if (cur && cur.c === cod) { cur = null; store.set('session.curVerif', null); document.getElementById('form-area').style.display = 'none'; }
  renderSession();
  scheduleSync();
...
async function activateSession(id, titolo, dataVerifica, utenteId = null) {
  ...
  cur = null; store.set('session.curVerif', null);
